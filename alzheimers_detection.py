...
def cv2_read_path(set_type, stages, img_counts):
    ...
    for stage, img_count in zip(stages, img_counts):
...
all_counts = [2559, 1791, 716, 51]

trainX, trainy = cv2_read_path(set_type='train', stages=all_stages, img_counts=all_counts)
...
for n in range(1,21):
    show_sample(trainX, -n)
    print(f'Label: {trainy[-n]}')
